...
from utils import escape_markdown, parse_filename, DOWNLOAD_PATH, fetch_root_dir_id
...
def load_data(context: CallbackContext):
    ...
    stats_data = {"downloaded": 0, "uploaded": 0}
    links_data = {}
    if os.path.exists(STATS_FILE):
        try:
            with open(STATS_FILE, 'r') as f: data = json.load(f)
            stats_data = data.get("stats", stats_data)
            links_data = data.get("saved_links", links_data)
        except (json.JSONDecodeError, IOError): pass
    context.bot_data['stats'] = stats_data
    context.bot_data['saved_links'] = links_data
    def save_stats_func():
...
def info_updater_thread(update: Update, context: CallbackContext, user_id: int):
    ...
    while True:
        ...
        if current_text and current_text != last_text:
            try:
                context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=message_id,
                                              text=current_text, parse_mode=ParseMode.MARKDOWN_V2)
                last_text = current_text
            except BadRequest as e:
                if "Message is not modified" not in str(e): print(f"Info update error: {e}"); break
            except Exception as e: print(f"Unexpected info update error: {e}"); break
        time.sleep(2)
...
def info_command(update: Update, context: CallbackContext) -> None:
    ...
    with task_lock:
        if user_id in ACTIVE_TASKS:
            if not ACTIVE_TASKS[user_id].get('info_thread_running', False):
...
def receive_link(update: Update, context: CallbackContext) -> int:
    ...
    keyboard = [[InlineKeyboardButton("1. Full Name", callback_data='full')],
                [InlineKeyboardButton("2. Smart Name", callback_data='smart')],
                [InlineKeyboardButton("3. Short Name", callback_data='short')],
                [InlineKeyboardButton("4. Custom Name", callback_data='custom')]]
...
def custom_name_received(update: Update, context: CallbackContext) -> int:
    custom_name = update.message.text; original_filename = context.user_data['original_filename']
    ext = os.path.splitext(original_filename)[1]
...
def main() -> None:
    ...
    if not all([BOT_TOKEN, BUZZHEAVIER_ACCOUNT_ID]): print("CRITICAL: BOT_TOKEN or BUZZHEAVIER_ACCOUNT_ID not found in .env file."); sys.exit(1)
    updater = Updater(BOT_TOKEN); dispatcher = updater.dispatcher
    load_data(dispatcher)
    print("Fetching BuzzHeavier Root Directory ID...")
    BUZZHEAVIER_ROOT_DIR_ID = fetch_root_dir_id(BUZZHEAVIER_ACCOUNT_ID)
    if not BUZZHEAVIER_ROOT_DIR_ID: print("CRITICAL: Could not fetch BuzzHeavier Root ID. Check account ID and connectivity."); sys.exit(1)
    print(f"Successfully fetched Root ID: {BUZZHEAVIER_ROOT_DIR_ID}")
